...
class EventFFDepthAnythingV2(nn.Module):
    ...
    def forward(
        ...
        resized = F.interpolate(
            field, (self.size, self.size), mode="bilinear", align_corners=False
        )
    ...
    def infer_image(
        ...
        fh, fw = get_resize_shapes(h, w, self.size, 14)

        field = self.field(events, counts)
        field = field[0, :, cparams[0] : cparams[2], cparams[1] : cparams[3]]
        resized = F.interpolate(
            field.unsqueeze(0), (fh, fw), mode="bilinear", align_corners=False
        )
        pred = self.dav2(resized).unsqueeze(1)
        pred = F.interpolate(pred, (h, w), mode="bilinear", align_corners=True)
        return pred[0, 0], field
